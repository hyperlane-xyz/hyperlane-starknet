...
fn setup_dex_collateral() -> (Setup, IMockParadexDexDispatcher, IHypERC20TestDispatcher) {
    let setup = setup();
    ...
    let (remote_dex_collateral_address, _) = hyp_erc20_dex_collateral_contract.deploy(@constructor_args).unwrap();
    let remote_dex_collateral = IHypERC20TestDispatcher { contract_address: remote_dex_collateral_address };
    dex.set_hyperlane_token(setup.primary_token.contract_address);
    ...
    (setup, dex, remote_dex_collateral)
}
...
fn test_dex_contract_setup() {
    let (setup, dex, collateral) = setup_dex_collateral();
    ...
    assert_eq!(deposit_token, setup.primary_token.contract_address, "Collateral token not set");
}
